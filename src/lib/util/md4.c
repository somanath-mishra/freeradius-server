...
{
	EVP_MD_CTX *md_ctx = talloc_get_type_abort(ctx, EVP_MD_CTX);
...
static fr_md4_ctx_t *fr_md4_openssl_ctx_alloc(bool thread_local)
{
	...
	if (thread_local) {
		...
		init:
			talloc_set_type(md_ctx, EVP_MD_CTX);
			EVP_DigestInit_ex(md_ctx, EVP_md4(), NULL);
		} else {
			md_ctx = md4_ctx;
		}
...
{
	EVP_DigestUpdate(talloc_get_type_abort(ctx, EVP_MD_CTX), in, inlen);
}
...
static void fr_md4_openssl_final(uint8_t out[static MD4_DIGEST_LENGTH], fr_md4_ctx_t *ctx)
{
	...
	EVP_DigestFinal(talloc_get_type_abort(ctx, EVP_MD_CTX), out, &len);
